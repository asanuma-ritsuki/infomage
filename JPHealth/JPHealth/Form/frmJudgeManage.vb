...
#Region "オブジェクトイベント"
	...
	''' <param name="sender"></param>
	''' <param name="e"></param>
	Private Sub btnTrackedChanges_Click(sender As Object, e As EventArgs) Handles btnTrackedChanges.Click

		Dim f As New frmTrackedChanges
		f.ShowDialog()
		SearchGrid()
...
#Region "プライベートメソッド"
	...
	Private Sub SearchGrid()
		...
		Try
			...
			strSQL &= "ISNULL(印刷日時, '1900/01/01') AS 印刷日時, ISNULL(再印刷日時, '1900/01/01') AS 再印刷日時, "
			...
			End If
			'印刷フラグ
			If Me.cmbPrinted.SelectedIndex = 1 Then
				'フラグあり
				strSQL &= strWhere & "ISNULL(印刷日時, '1900/01/01') != '1900/01/01'"
				strWhere = " AND "
			ElseIf Me.cmbPrinted.SelectedIndex = 2 Then
				'フラグなし
				strSQL &= strWhere & "ISNULL(印刷日時, '1900/01/01') = '1900/01/01'"
				strWhere = " AND "
			End If
			...
			For iRow As Integer = 0 To dt.Rows.Count - 1
				...
				Me.C1FGridResult(iRecordCount, "印刷日時") = IIf(dt.Rows(iRow)("印刷日時") = "1900/01/01", "", dt.Rows(iRow)("印刷日時"))
				Me.C1FGridResult(iRecordCount, "再印刷日時") = IIf(dt.Rows(iRow)("再印刷日時") = "1900/01/01", "", dt.Rows(iRow)("再印刷日時"))
	...
	Private Sub SearchCount()
		...
		Try
			...
			strSQL &= "AND ISNULL(印刷日時, '1900/01/01') = '1900/01/01' AND ISNULL(完了日時, '1900/01/01) = '1900/01/01 "
			...
			'印刷件数
			'完了日時は考慮しない
			strSQL = "SELECT COUNT(印刷日時) FROM T_判定票管理 "
			strSQL &= "WHERE ISNULL(印刷日時, '1900/01/01) != '1900/01/01 AND ISNULL(削除日時, '1900/01/01') = '1900/01/01'"
		...
		For iRow As Integer = 0 To Me.C1FGridResult.Rows.Count - 1
			...
			If IsNull(Me.C1FGridResult(iRow, "削除日時")) Then
				...
				If IsNull(Me.C1FGridResult(iRow, "完了日時")) Then
					'印刷日がNULL
					If IsNull(Me.C1FGridResult(iRow, "印刷日時")) Then
						'修正済日時がNULL
