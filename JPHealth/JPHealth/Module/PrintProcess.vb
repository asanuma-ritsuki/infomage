Module PrintProcess
		...
		Try
			...
			For iRow As Integer = 0 To dtOffice.Rows.Count - 1
				...
				If dtWeightHeader.Rows.Count > 0 Then
					...
					strSQL &= "AND T1.会社コード = '" & dtOffice.Rows(iRow)("会社コード") & "' "
					strSQL &= "AND T1.所属事業所コード = '" & dtOffice.Rows(iRow)("所属事業所コード") & "' "
					strSQL &= "ORDER BY CASE WHEN T1.所属部名 IS NULL THEN 1 ELSE 0 END, "
					strSQL &= "CASE WHEN T1.所属課名 IS NULL THEN 1 ELSE 0 END, "
					strSQL &= "T1.社員コード"
					...
					For iPage As Integer = 1 To iCheckupListPage
						...
						For iRec As Integer = iStartRec To iEndRec
							...
							If dtCheckupList.Rows(iRec)("就業区分") = "就業制限" Or dtCheckupList.Rows(iRec)("就業区分") = "要休業" Then
								strMark = "○"
							End If
					...
					For i As Integer = 0 To dtCheckupList.Rows.Count - 1
						...
						strSQL &= ", 1" '帳票種別詳細
						strSQL &= ", ''"    '分数(NULL)
						'QRコードの算出(システムID(15)＋帳票種別ID(1))
						strQRCodeWithoutCD = dtCheckupList.Rows(i)("システムID") & "4"
						strCD = ""
					...
					For i As Integer = 0 To dtLeaf.Rows.Count - 1
						...
						For j As Integer = 0 To dtLeafSystemID.Rows.Count - 1
							...
							'QRコードの算出(システムID(15)＋帳票種別ID(1))
							'QRコードの算出(システムID(15)＋帳票種別ID(1))
							strQRCodeWithoutCD = dtCheckupList.Rows(i)("システムID") & "5"
							strCD = ""
				...
				Else
					...
					strSQL &= "AND 所属事業所コード = '" & dtOffice.Rows(iRow)("所属事業所コード") & "' "
					strSQL &= "ORDER BY CASE WHEN 所属部名 IS NULL THEN 1 ELSE 0 END, "
					strSQL &= "CASE WHEN 所属課名 IS NULL THEN 1 ELSE 0 END, "
					strSQL &= "社員コード"
					...
					For i As Integer = 0 To dtPrintID.Rows.Count - 1
						...
						'==================================================
						strSQL = "SELECT T1.ロットID, T1.レコード番号, T1.システムID, T2.会社, T2.所属事業所, T1.所属部名, T1.所属課名, T2.氏名姓 + ' ' + T2.氏名名 AS 氏名, "
						strSQL &= "T2.受診日, T3.変換健診種別 AS 健診種別, T2.就業区分, T4.印刷ID "
						strSQL &= "FROM T_判定票管理 AS T1 INNER JOIN "
						strSQL &= "T_判定票 AS T2 ON T1.ロットID = T2.ロットID AND T1.レコード番号 = T2.レコード番号 INNER JOIN "
						...
						For j As Integer = 0 To dtCheckupList.Rows.Count - 1
							...
							strSQL &= ", ''"    '分数(NULL)
							'QRコードの算出(システムID(15)＋帳票種別ID(1))
							strQRCodeWithoutCD = dtCheckupList.Rows(j)("システムID") & "4"
							strCD = ""
							blnResult = Modulus10(strQRCodeWithoutCD, 4, strCD)
							strQRCode = strQRCodeWithoutCD & strCD
							strSQL &= ", '" & strQRCode & "'"
						...
						For k As Integer = 0 To dtLeaf.Rows.Count - 1
							...
							For j As Integer = 0 To dtLeafSystemID.Rows.Count - 1
								...
								'QRコードの算出(システムID(15)＋帳票種別ID(1))
								'QRコードの算出(システムID(15)＋帳票種別ID(1))
								strQRCodeWithoutCD = dtCheckupList.Rows(k)("システムID") & "5"
								strCD = ""
								blnResult = Modulus10(strQRCodeWithoutCD, 5, strCD)
								strQRCode = strQRCodeWithoutCD & strCD
								strSQL &= ", '" & strQRCode & "'"
				...
				For i As Integer = 0 To dtCheckupPrint.Rows.Count - 1
					...
					strSQL &= ", " & dtCheckupPrint.Rows(i)("受診年齢")
					...
					strSQL &= ", '" & WhichUpDown(dtCheckupPrint.Rows(i)("血圧1回収縮期"), dtCheckupPrint.Rows(i)("血圧1回収縮期"), dtCheckupPrint.Rows(i)("血圧1回収縮期")) & "'"
					...
					'QRコード
					'QRコードの算出(会社コード(4)＋所属事業所コード(6)＋帳票種別ID(1)＋ページ数(4))
					strQRCodeWithoutCD = dtCheckupPrint.Rows(i)("システムID") & "4"
					strCD = ""
					blnResult = Modulus10(strQRCodeWithoutCD, 4, strCD)
					strQRCode = strQRCodeWithoutCD & strCD
					strSQL &= ", '" & strQRCode & "'"
				...
				For iSystemID As Integer = 0 To dtLeadletSystemID.Rows.Count - 1
					...
					For i As Integer = 0 To dtLeafletPrint.Rows.Count - 1
						...
						strSQL &= ", '" & strWAS6 & "'"
						'QRコードの算出(システムID(15)＋帳票種別ID(1))
						strQRCodeWithoutCD = dtLeafletPrint.Rows(i)("システムID") & "5"
						strCD = ""
						blnResult = Modulus10(strQRCodeWithoutCD, 5, strCD)
						strQRCode = strQRCodeWithoutCD & strCD
	...
	Private Sub PrintLabel(ByVal strLotID As String)
	...
	Private Sub PrintCheckup(ByVal strLotID As String, Optional ByVal iRecordNumber As Integer = 0)
		...
		Try
			...
			If Not iRecordNumber = 0 Then
				...
				For iRow As Integer = 0 To dtPrintManage.Rows.Count - 1
					...
					strSQL &= "T1.会社, T1.所属事業所, T1.所属部名, T1.所属課名, T1.氏名, T1.受診日, T1.健診種別, T1.意見書発行, T1.判定票要押印 "
					strSQL &= "FROM T_対象者一覧印刷 AS T1 INNE JOIN "
					...
					strSQL &= "T1.所属部名, T1.所属課名, T1.役職名, T1.性別, T1.採用年月日, T1.生年月日, T1.健診種別, T1.受診日, T1.身長, T1.体重, "
					...
					strSQL &= "T_判定票管理 AS T3 ON T1.ロットID = T3.ロットID AND T1.レコード番号 = T2.レコード番号 "
					strSQL &= "WHERE T1.ロットID = '" & strLotID & "' "
	...
	''' <param name="strLotID">対象ロットID</param>
	''' <param name="iRecordNumber">単体で印刷したいときにレコード番号を指定する</param>
	Private Sub PrintLeaflet(ByVal strLotID As String, Optional ByVal iRecordNumber As Integer = 0)
		...
		Try
			...
			If Not iRecordNumber = 0 Then
				...
				For iRow As Integer = 0 To dtPrintManage.Rows.Count - 1
					...
					'重量ヘッダシート印刷
					'==================================================
					'先に対象のT_リーフレット印刷を取得して、1レコード以上存在した場合に重量ヘッダを出力する
					...
					strSQL &= "ORDER BY CASE WHEN T1.所属部課名 IS NULL THEN 1 ELSE 0 END, "
					strSQL &= "T3.社員コード"
