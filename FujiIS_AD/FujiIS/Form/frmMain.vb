...
#Region "オブジェクトイベント"
	...
	Private Sub btnStart_Click(sender As System.Object, e As System.EventArgs) Handles btnStart.Click
		...
		Try
			...
			strSQL = "SELECT 処理端末 FROM M_冊子広告 "
			...
			'パスを作成する
			XmlSettings.LoadFromXmlFile()
			Dim strTargetPath As String = XmlSettings.Instance.ImagePath
			'パスの存在確認
			...
			'開始日時、処理端末の保存
			strSQL = "UPDATE M_冊子広告 SET 開始日時 = '" & Date.Now.ToString("yyyy/MM/dd HH:mm:ss") & "', "
			...
			strSQL = "SELECT 冊子名 + ' ' + 年号 + ' ' + Vol + ' ' + No AS 冊子名 FROM M_冊子広告 "
			strSQL &= "WHERE ID = '" & Me.cmbBookletID.SelectedValue & "'"
		...
		Try
			...
			If RegistData() Then
				...
				If LinkProcess = 1 Then
					strSQL = "UPDATE M_冊子広告 SET 終了日時 = NULL "
					strSQL &= "WHERE ID = '" & Me.cmbBookletID.SelectedValue & "'"
				ElseIf LinkProcess = 2 Then
					strSQL = "UPDATE M_冊子広告 SET 終了日時2次 = NULL "
					strSQL &= "WHERE ID = '" & Me.cmbBookletID.SelectedValue & "'"
				End If
				sqlProcess.DB_UPDATE(strSQL)
	...
	Private Sub btnFinish_Click(sender As System.Object, e As System.EventArgs) Handles btnFinish.Click
		...
		Try
			...
			If RegistData() Then
				...
				If LinkProcess = 1 Then
					strSQL = "UPDATE M_冊子広告 SET 終了日時 = '" & Date.Now.ToString("yyyy/MM/dd HH:mm:ss") & "' "
					strSQL &= "WHERE ID = '" & Me.cmbBookletID.SelectedValue & "'"
				ElseIf LinkProcess = 2 Then
					strSQL = "UPDATE M_冊子広告 SET 終了日時2次 = '" & Date.Now.ToString("yyyy/MM/dd HH:mm:ss") & "' "
					strSQL &= "WHERE ID = '" & Me.cmbBookletID.SelectedValue & "'"
				End If
				sqlProcess.DB_UPDATE(strSQL)
...
#Region "プライベートメソッド"
	...
	Private Sub SearchGrid()
		...
		'2017/05/08
		'T_目次広告よりIDを検索してそれぞれのフォルダ番号＋ファイル名でグリッドにセットする
		Dim strSQL As String = ""
		Dim sqlProcess As New SQLProcess
		...
		Try
			...
			Dim strBookletID As String = Me.cmbBookletID.SelectedValue
			strSQL = "SELECT 目次タイトル, 著者名, フォルダ番号, リンクファイル名, 対象イメージ無, 備考, うるる備考, 連番 FROM T_目次広告 "
			strSQL &= "WHERE ID = '" & strBookletID & "' "
			...
			'C1FGridResultに列挙
			Me.C1FGridResult.Rows.Count = dt.Rows.Count + 1
			Me.C1FGridMokuji.Rows.Count = dt.Rows.Count + 1
			For iRow As Integer = 0 To dt.Rows.Count - 1
				'T_目次広告をもとにパスを作成してグリッドにセットしていく
				Dim strFile As String = strTargetPath & "\" & dt.Rows(iRow)("フォルダ番号") & "\" & dt.Rows(iRow)("リンクファイル名") & ".pdf"
				Me.C1FGridResult(iRow + 1, "No") = iRow + 1
				Me.C1FGridResult(iRow + 1, "ファイル名") = System.IO.Path.GetFileNameWithoutExtension(strFile)
				Me.C1FGridResult(iRow + 1, "パス") = strFile

				'目次グリッド
				Me.C1FGridMokuji(iRow + 1, "No") = iRow + 1
				Me.C1FGridMokuji(iRow + 1, "目次タイトル") = dt.Rows(iRow)("目次タイトル")
				Me.C1FGridMokuji(iRow + 1, "著者名") = dt.Rows(iRow)("著者名")
				Me.C1FGridMokuji(iRow + 1, "ファイル名") = System.IO.Path.GetFileNameWithoutExtension(strFile)
				Me.C1FGridMokuji(iRow + 1, "備考") = dt.Rows(iRow)("備考")
				Me.C1FGridMokuji(iRow + 1, "うるる備考") = dt.Rows(iRow)("うるる備考")
				Me.C1FGridMokuji(iRow + 1, "対象イメージ無") = IIf(dt.Rows(iRow)("対象イメージ無") = 0, False, True)
				Me.C1FGridMokuji(iRow + 1, "連番") = dt.Rows(iRow)("連番")
			Next
			''パスを作成する
			'XmlSettings.LoadFromXmlFile()
			...
			'ステータスバーに画像数を表示
			Me.lblImageCount.Text = Me.C1FGridResult.Rows.Count.ToString & "images"
	...
	Private Sub ClearValue()
		...
		Try
			strSQL = "UPDATE M_冊子広告 SET 処理端末 = N'' "
		...
		Try
			...
			Select Case iCol
				...
				Case 3
					...
					'		If Not IsNull(Me.C1FGridMokuji(i, "ファイル名")) Then
					'			iTargetRow = i
					'			Exit For
					'		End If
					'	Next
	...
	Private Sub SetLink()
		...
		Dim iRow As Integer = Me.C1FGridMokuji.Row
		Me.C1FGridMokuji(iRow, "ファイル名") = Me.C1FGridResult(Me.C1FGridResult.Row, "ファイル名")
		Dim iTargetRow As Integer = 0
		'===================================================================
		''2017/03/21
	...
	Private Function RegistData() As Boolean
		...
		Try
			...
			strSQL = "SELECT ISNULL(MAX(履歴ID), 0) + 1 AS 履歴ID FROM T_目次広告バックアップ"
			Dim strRirekiID As String = sqlProcess.DB_EXECUTE_SCALAR(strSQL)
			'グリッドより1レコードずつINSERTする
			For iRow As Integer = 1 To Me.C1FGridMokuji.Rows.Count - 1
				'該当連番の必要項目をT_目次広告より取得してINSERT文にセットする
				strSQL = "SELECT 連番, レコード番号, 冊子名, 元ファイル名, フォルダ番号, ファイル名, ハイパーリンク "
				strSQL &= "FROM T_目次広告 "
				strSQL &= "WHERE 連番 = " & Me.C1FGridMokuji.Rows(iRow)("連番")
				Dim dt As DataTable = sqlProcess.DB_SELECT_DATATABLE(strSQL)

				If dt.Rows.Count >= 0 Then
					'レコードが存在していたらデータをセットする
					strSQL = "INSERT INTO T_目次広告バックアップ (履歴ID, 履歴ID連番, 端末名, 連番, ID, レコード番号, 冊子名, 目次タイトル, 著者名, 元ファイル名, フォルダ番号, ファイル名, リンクファイル名, ハイパーリンク, 対象イメージ無, 備考, うるる備考, 登録日時) "
					strSQL &= "VALUES("
					strSQL &= strRirekiID   '履歴ID
					strSQL &= ", " & iRow   '履歴ID連番
					strSQL &= ", N'" & strHostName & "'"    '端末名
					strSQL &= ", " & dt.Rows(0)("連番")   '連番
					strSQL &= ", N'" & strBookletID & "'"   'ID
					strSQL &= ", " & dt.Rows(0)("レコード番号")   'レコード番号
					strSQL &= ", N'" & dt.Rows(0)("冊子名") & "'"  '冊子名
					...
					Dim strChosha As String = Me.C1FGridMokuji.Rows(iRow)("著者名")
					If IsNull(strChosha) Then
						strSQL &= ", N''"
					Else
						strSQL &= ", N'" & strChosha.Replace("'", "''") & "'"
					End If
					strSQL &= ", N'" & dt.Rows(0)("元ファイル名") & "'"   '元ファイル名
					'フォルダ番号、ファイル名は今現在セットされているリンクファイル名をバラしてセットする
					strSQL &= ", N'" & IIf(IsNull(Me.C1FGridMokuji.Rows(iRow)("ファイル名")) = True, "", Strings.Left(Me.C1FGridMokuji.Rows(iRow)("ファイル名"), 6)) & "'"   'フォルダ番号
					strSQL &= ", N'" & IIf(IsNull(Me.C1FGridMokuji.Rows(iRow)("ファイル名")) = True, "", Strings.Right(Me.C1FGridMokuji.Rows(iRow)("ファイル名"), 3)) & "'"   'ファイル名
					strSQL &= ", N'" & Me.C1FGridMokuji.Rows(iRow)("ファイル名") & "'"   'リンクファイル名
					strSQL &= ", N'" & dt.Rows(0)("ハイパーリンク") & "'"  'ハイパーリンク
					strSQL &= ", " & IIf(IsNull(Me.C1FGridMokuji.Rows(iRow)("ファイル名")), 1, 0)   '対象イメージ無※リンクファイル名がなかったらTRUE
					strSQL &= ", N'" & Me.C1FGridMokuji.Rows(iRow)("備考") & "'"  '備考
					strSQL &= ", N'" & Me.C1FGridMokuji.Rows(iRow)("うるる備考") & "'"   'うるる備考
					strSQL &= ", '" & strBackupDate & "')"   '登録日時
					sqlProcess.DB_UPDATE(strSQL)
				Else
					'レコードが存在しなかった場合（レコード追加）必要項目は空文字にする
					strSQL = "INSERT INTO T_目次広告バックアップ (履歴ID, 履歴ID連番, 端末名, 連番, ID, レコード番号, 冊子名, 目次タイトル, 著者名, 元ファイル名, フォルダ番号, ファイル名, リンクファイル名, ハイパーリンク, 対象イメージ無, 備考, うるる備考, 登録日時) "
					strSQL &= "VALUES("
					strSQL &= strRirekiID   '履歴ID
					strSQL &= ", " & iRow   '履歴ID連番
			...
			strSQL = "DELETE FROM T_目次広告 WHERE ID = '" & strBookletID & "'"
			sqlProcess.DB_UPDATE(strSQL)
			strSQL = "INSERT INTO T_目次広告 (連番, ID, レコード番号, 冊子名, 目次タイトル, 著者名, 元ファイル名, フォルダ番号, ファイル名, リンクファイル名, ハイパーリンク, 対象イメージ無, 備考, うるる備考) "
			strSQL &= "SELECT 連番, ID, レコード番号, 冊子名, 目次タイトル, 著者名, 元ファイル名, フォルダ番号, ファイル名, リンクファイル名, ハイパーリンク, 対象イメージ無, 備考, うるる備考 FROM T_目次広告バックアップ "
