...
#Region "オブジェクトイベント"
        ...
        Try
            ...
            For Each strFile As String In strTargetFiles
                ...
                If ExtractZIP(strFile, strDestFolder, Me.txtPasswordDate.Text) Then
                    ...
                    Using parser As New CSVParser(strCheckupCSV(0), System.Text.Encoding.GetEncoding("Shift-JIS"))
                        ...
                        While Not parser.EndOfData
                            ...
                            Else
                                ...
                                '2019/04/16
                                '採用年月日の方が後、かつ健診種別が「雇入時*」の場合に変更
                                If iParse > 0 And row(16).IndexOf("雇入時") >= 0 Then
                                    '採用年月日のほうが後、健診種別が「雇入時*」だった
                    ...
                    '判定票、印影印字判断処理
                    '==================================================
                    WriteLstResult(Me.lstResult, "印影判断処理開始（判定票）")

                    '2019/05/10
                    '会場局コードはT_判定票(実データ)を参照する
                    strSQL = "SELECT T1.ロットID, T1.レコード番号, T1.会社コード, T1.所属事業所コード, T1.社員コード, T2.氏名姓 + T2.氏名名 AS 氏名, T1.年度, T1.健診種別, T2.判定医師名 AS 産業医, T2.健診実施医師名 AS 判定医, T2.会場局コード "
                    strSQL &= "FROM T_判定票管理 AS T1 INNER JOIN "
                    strSQL &= "T_判定票 AS T2 ON T1.ロットID = T2.ロットID AND T1.レコード番号 = T2.レコード番号 INNER JOIN "
                    strSQL &= "M_局所 AS T3 ON T1.会社コード = T3.会社コード AND T1.所属事業所コード = T3.局所コード "
                    strSQL &= "WHERE T1.ロットID = '" & strLotID & "' "
                    strSQL &= "ORDER BY T1.ロットID, T1.レコード番号"
                    Dim dtImprint As DataTable = sqlProcess.DB_SELECT_DATATABLE(strSQL)
                    For iImprint As Integer = 0 To dtImprint.Rows.Count - 1
                        '年度の判断
                        '2018年度以前は空レコードをT_医師にINSERT
                        If CInt(dtImprint.Rows(iImprint)("年度")) <= 2018 Then
                            strSQL = "INSERT INTO T_医師(ロットID, レコード番号, 産業医ID, 判定医ID, 対象フラグ, 有効フラグ) VALUES("
                            strSQL &= "'" & dtImprint.Rows(iImprint)("ロットID") & "'"
                            strSQL &= ", " & dtImprint.Rows(iImprint)("レコード番号")
                            strSQL &= ", 0, 0, 0, 1)"  '産業医ID、判定医ID、対象フラグは0、有効フラグは1
                            sqlProcess.DB_UPDATE(strSQL)
                        Else
                            '2019年度以降
                            '会場局コードが"999999"(人間ドック)か
                            Dim strSangyouName As String = dtImprint.Rows(iImprint)("産業医").ToString.Replace(" ", "").Replace("　", "")
                            Dim strHanteiName As String = dtImprint.Rows(iImprint)("判定医").ToString.Replace(" ", "").Replace("　", "")
                            Dim iSangyouID As Integer = 0
                            Dim iHanteiID As Integer = 0
                            If dtImprint.Rows(iImprint)("会場局コード") = "999999" Then
                                '"999999"(人間ドック)だった
                                '産業医のみ存在チェック
                                strSQL = "SELECT 産業医ID FROM M_産業医 "
                                '全角、半角スペースを取り除いて比較する
                                strSQL &= "WHERE REPLACE(REPLACE(医師名, ' ', ''), '　', '') = '" & strSangyouName & "'"
                                Dim dtDoctor As DataTable = sqlProcess.DB_SELECT_DATATABLE(strSQL)
                                If dtDoctor.Rows.Count = 0 Then
                                    '●不備に挙げる（産業医存在チェックエラー）
                                    'エラーが発生した場合
                                    iIncompleteCount += 1
                                    WriteLstResult(Me.lstResult, "該当の産業医がマスタに存在しませんでした（人間ドック）", ResultMark.ErrorMark)
                                    WriteLstResult(Me.lstResult, "会社コード：" & dtImprint.Rows(iImprint)("会社コード") & " 所属事業所コード：" & dtImprint.Rows(iImprint)("所属事業所コード") & " 社員コード：" & dtImprint.Rows(iImprint)("社員コード") & " 産業医：" & dtImprint.Rows(iImprint)("産業医"), ResultMark.ErrorMark)
                                    WriteNoGoodImprint(dtImprint.Rows(iImprint)("ロットID"), dtImprint.Rows(iImprint)("レコード番号"), dtImprint.Rows(iImprint)("産業医"), ImprintErrorDoctor.SangyouDoctor)
                                    '2019/04/23
                                    '該当レコードを[T_判定票_印影不備]に移動する
                                    MoveRecordImprint(dtImprint.Rows(iImprint)("ロットID"), dtImprint.Rows(iImprint)("レコード番号"), "T_判定票", "T_判定票_印影不備")

                                    strSQL = "INSERT INTO T_医師(ロットID, レコード番号, 産業医ID, 判定医ID, 対象フラグ, 有効フラグ) VALUES("
                                    strSQL &= "'" & dtImprint.Rows(iImprint)("ロットID") & "'"
                                    strSQL &= ", " & dtImprint.Rows(iImprint)("レコード番号")
                                    strSQL &= ", 0, 0, 0, 0)"  '産業医ID、判定医ID、対象フラグ、有効フラグ全て0
                                    sqlProcess.DB_UPDATE(strSQL)
                                Else
                                    iSangyouID = dtDoctor.Rows(0)("産業医ID")
                                    strSQL = "INSERT INTO T_医師(ロットID, レコード番号, 産業医ID, 判定医ID, 対象フラグ, 有効フラグ) VALUES("
                                    strSQL &= "'" & dtImprint.Rows(iImprint)("ロットID") & "'"
                                    strSQL &= ", " & dtImprint.Rows(iImprint)("レコード番号")
                                    strSQL &= ", " & iSangyouID
                                    strSQL &= ", 0, 1, 1)"  '判定医IDは0、対象フラグ、有効フラグは1
                                    sqlProcess.DB_UPDATE(strSQL)
                                End If
                            Else
                                '2019年度以降で人間ドック以外
                                '産業医、判定医の存在チェック
                                Dim blnDoctorError As Boolean = False
                                strSQL = "SELECT 産業医ID FROM M_産業医 "
                                '全角、半角スペースを取り除いて比較する
                                strSQL &= "WHERE REPLACE(REPLACE(医師名, ' ', ''), '　', '') = '" & strSangyouName & "'"
                                Dim dtDoctor As DataTable = sqlProcess.DB_SELECT_DATATABLE(strSQL)
                                If dtDoctor.Rows.Count = 0 Then
                                    '●不備に挙げる（産業医存在チェックエラー）
                                    WriteLstResult(Me.lstResult, "該当の産業医がマスタに存在しませんでした", ResultMark.ErrorMark)
                                    WriteLstResult(Me.lstResult, "会社コード：" & dtImprint.Rows(iImprint)("会社コード") & " 所属事業所コード：" & dtImprint.Rows(iImprint)("所属事業所コード") & " 社員コード：" & dtImprint.Rows(iImprint)("社員コード") & " 産業医：" & dtImprint.Rows(iImprint)("産業医"), ResultMark.ErrorMark)
                                    WriteNoGoodImprint(dtImprint.Rows(iImprint)("ロットID"), dtImprint.Rows(iImprint)("レコード番号"), dtImprint.Rows(iImprint)("産業医"), ImprintErrorDoctor.SangyouDoctor)
                                    blnDoctorError = True
                                    ''2019/04/23
                                ...
                                Else
                                    iSangyouID = dtDoctor.Rows(0)("産業医ID")
                                End If
                                strSQL = "SELECT 判定医ID FROM M_判定医 "
                                '全角、半角スペースを取り除いて比較する
                                strSQL &= "WHERE REPLACE(REPLACE(医師名, ' ', ''), '　', '') = '" & strHanteiName & "'"
                                dtDoctor = sqlProcess.DB_SELECT_DATATABLE(strSQL)
                                If dtDoctor.Rows.Count = 0 Then
                                    '●不備に挙げる（判定医存在チェックエラー）
                                    WriteLstResult(Me.lstResult, "該当の判定医がマスタに存在しませんでした", ResultMark.ErrorMark)
                                    WriteLstResult(Me.lstResult, "会社コード：" & dtImprint.Rows(iImprint)("会社コード") & " 所属事業所コード：" & dtImprint.Rows(iImprint)("所属事業所コード") & " 社員コード：" & dtImprint.Rows(iImprint)("社員コード") & " 判定医：" & dtImprint.Rows(iImprint)("判定医"), ResultMark.ErrorMark)
                                    WriteNoGoodImprint(dtImprint.Rows(iImprint)("ロットID"), dtImprint.Rows(iImprint)("レコード番号"), dtImprint.Rows(iImprint)("判定医"), ImprintErrorDoctor.HanteiDoctor)
                                    blnDoctorError = True
                                    ''2019/04/23
                                    ''該当レコードを[T_判定票_印影不備]に移動する
                                    'MoveRecordImprint(dtImprint.Rows(iImprint)("ロットID"), dtImprint.Rows(iImprint)("レコード番号"), "T_判定票", "T_判定票_印影不備")
                                Else
                                    iHanteiID = dtDoctor.Rows(0)("判定医ID")
                                End If
                                If blnDoctorError Then
                                    'エラーが発生していたら
                                    iIncompleteCount += 1
                                    strSQL = "INSERT INTO T_医師(ロットID, レコード番号, 産業医ID, 判定医ID, 対象フラグ, 有効フラグ) VALUES("
                                    strSQL &= "'" & dtImprint.Rows(iImprint)("ロットID") & "'"
                                    ...
                                    sqlProcess.DB_UPDATE(strSQL)
                                    '2019/04/23
                                    '該当レコードを[T_判定票_印影不備]に移動する、T_判定票管理からレコードを削除する
                                    MoveRecordImprint(dtImprint.Rows(iImprint)("ロットID"), dtImprint.Rows(iImprint)("レコード番号"), "T_判定票", "T_判定票_印影不備")
                                Else
                                    'エラーなし
                                    strSQL = "INSERT INTO T_医師(ロットID, レコード番号, 産業医ID, 判定医ID, 対象フラグ, 有効フラグ) VALUES("
                    ...
                    WriteLstResult(Me.lstResult, "印影判断処理完了（判定票）")
                    WriteLstResult(Me.lstResult, "--------------------------------------------------------------------------------", ResultMark.InformationMark)
                    '==================================================
                    ...
                    Else
                        ...
                        Using parser As New CSVParser(strLeafletCSV(0), System.Text.Encoding.GetEncoding("Shift-JIS"))
                            ...
                            While Not parser.EndOfData
                                ...
                                '2019/04/22
                                '紐付け時に受診日を検索条件に追加
                                strSQL &= "AND T2.受診日 = '" & row(17) & "'"
                                dt = sqlProcess.DB_SELECT_DATATABLE(strSQL)
                                If dt.Rows.Count = 0 Then
                                    '2019/05/20
                                    'T_判定票内のレコードに存在しなかった場合、T_判定票_印影不備もチェックする
                                    strSQL = "SELECT レコード番号 "
                                    strSQL &= "FROM T_判定票_印影不備 "
                                    strSQL &= "WHERE ロットID = '" & strLotID & "' "
                                    strSQL &= "AND システムID = '" & row(0) & "' "
                                    strSQL &= "AND 受診日 = '" & row(17) & "'"
                                    dt = sqlProcess.DB_SELECT_DATATABLE(strSQL)
                                    If dt.Rows.Count = 0 Then
                            ...
                            '印影判断処理（リーフレット）
                            WriteLstResult(Me.lstResult, "--------------------------------------------------------------------------------", ResultMark.InformationMark)
                            WriteLstResult(Me.lstResult, "印影判断処理開始（リーフレット）")
                            strSQL = "SELECT T1.ロットID, T1.レコード番号, T1.不備項目, T1.会社コード, T1.所属事業所コード, T1.社員コード, T2.判定医師名 AS 産業医, T2.健診実施医師名 AS 判定医 "
                            strSQL &= "FROM T_判定票_不備内容 AS T1 INNER JOIN "
                            '2019/05/09
                            '該当の判定票データは「T_判定票_印影不備」に移動済みなのでそちらを参照する
                            strSQL &= "T_判定票_印影不備 AS T2 ON T1.ロットID = T2.ロットID AND T1.レコード番号 = T2.レコード番号 "
                            strSQL &= "WHERE T1.ロットID = '" & strLotID & "' "
                            strSQL &= "AND T1.不備種別 = " & ErrorCategory.ImprintError
                            Dim dtImprintLeaflet As DataTable = sqlProcess.DB_SELECT_DATATABLE(strSQL)

                            For iImprintLeaflet As Integer = 0 To dtImprintLeaflet.Rows.Count - 1
                                '複数リーフレット対応
                                strSQL = "SELECT ロットID, レコード番号, 帳票タイプ FROM T_リーフレット "
                                strSQL &= "WHERE ロットID = '" & dtImprintLeaflet.Rows(iImprintLeaflet)("ロットID") & "' "
                                strSQL &= "AND レコード番号 = " & dtImprintLeaflet.Rows(iImprintLeaflet)("レコード番号")
                                dt = sqlProcess.DB_SELECT_DATATABLE(strSQL)
                                For iRow As Integer = 0 To dt.Rows.Count - 1
                                    'エラーが発生した場合
                                    iIncompleteCount += 1
                                    If dtImprintLeaflet.Rows(iImprintLeaflet)("不備項目") = "産業医" Then
                                        WriteLstResult(Me.lstResult, "印影不備が発生しました", ResultMark.WarningMark)
                                        WriteLstResult(Me.lstResult, "会社コード：" & dtImprintLeaflet.Rows(iImprintLeaflet)("会社コード") & " 所属事業所コード：" & dtImprintLeaflet.Rows(iImprintLeaflet)("所属事業所コード") & " 社員コード：" & dtImprintLeaflet.Rows(iImprintLeaflet)("社員コード") & " 項目名：産業医" & " 帳票タイプ：" & dt.Rows(iRow)("帳票タイプ"), ResultMark.WarningMark)
                                        WriteNoGoodImprintLeaflet(dtImprintLeaflet.Rows(iImprintLeaflet)("ロットID"), dtImprintLeaflet.Rows(iImprintLeaflet)("レコード番号"), dtImprintLeaflet.Rows(iImprintLeaflet)("産業医"), ImprintErrorDoctor.SangyouDoctor, dt.Rows(iRow)("帳票タイプ"))
                                        '2019/04/23
                                        '該当レコードを[T_リーフレット_印影不備]に移動する
                                        MoveRecordImprint(dtImprintLeaflet.Rows(iImprintLeaflet)("ロットID"), dtImprintLeaflet.Rows(iImprintLeaflet)("レコード番号"), "T_リーフレット", "T_リーフレット_印影不備")
                                    Else
                                        WriteLstResult(Me.lstResult, "印影不備が発生しました", ResultMark.WarningMark)
                                        WriteLstResult(Me.lstResult, "会社コード：" & dtImprintLeaflet.Rows(iImprintLeaflet)("会社コード") & " 所属事業所コード：" & dtImprintLeaflet.Rows(iImprintLeaflet)("所属事業所コード") & " 社員コード：" & dtImprintLeaflet.Rows(iImprintLeaflet)("社員コード") & " 項目名：判定医" & " 帳票タイプ：" & dt.Rows(iRow)("帳票タイプ"), ResultMark.WarningMark)
                                        WriteNoGoodImprintLeaflet(dtImprintLeaflet.Rows(iImprintLeaflet)("ロットID"), dtImprintLeaflet.Rows(iImprintLeaflet)("レコード番号"), dtImprintLeaflet.Rows(iImprintLeaflet)("判定医"), ImprintErrorDoctor.HanteiDoctor, dt.Rows(iRow)("帳票タイプ"))
                                        '2019/04/23
                                        '該当レコードを[T_リーフレット_印影不備]に移動する
                                        MoveRecordImprint(dtImprintLeaflet.Rows(iImprintLeaflet)("ロットID"), dtImprintLeaflet.Rows(iImprintLeaflet)("レコード番号"), "T_リーフレット", "T_リーフレット_印影不備")
                                    End If
                            ...
                            WriteLstResult(Me.lstResult, "印影判断処理完了（リーフレット）")
                            WriteLstResult(Me.lstResult, "--------------------------------------------------------------------------------", ResultMark.InformationMark)
                            '===============================================================
	...
	Private Sub btnDelete2_Click(sender As Object, e As EventArgs) Handles btnDelete2.Click
		...
		Try
			...
			strSQL &= "WHERE ロットID = '" & strLotID & "'"
			sqlProcess.DB_UPDATE(strSQL)
            strSQL &= "DELETE FROM T_判定票_印影不備 "
            strSQL &= "WHERE ロットID = '" & strLotID & "'"
            sqlProcess.DB_UPDATE(strSQL)
            strSQL &= "DELETE FROM T_リーフレット_印影不備 "
            strSQL &= "WHERE ロットID = '" & strLotID & "'"
            sqlProcess.DB_UPDATE(strSQL)
            strSQL &= "DELETE FROM T_医師 "
            strSQL &= "WHERE ロットID = '" & strLotID & "'"
            sqlProcess.DB_UPDATE(strSQL)
...
#Region "プライベートメソッド"
	...
	Private Sub WeightHeaderToExcel(ByVal strExcelFile As String, ByVal strLotID As String)
		...
		Try
            ...
            '2019/04/23
            '判定票印影不備、リーフレット印影不備件、リーフレット印影不備枚
            strSQL = "SELECT "
            strSQL &= "SUM(CASE WHEN CSVファイル LIKE 'Checkup%' THEN 1 ELSE 0 END) AS 判定票件数 "
            strSQL &= "FROM T_判定票_不備内容 "
            strSQL &= "WHERE ロットID = '" & strLotID & "' "
            strSQL &= "AND 不備種別 = 6 "
            strSQL &= "GROUP BY レコード番号"
            Dim iCheckupCount As Integer = sqlProcess.DB_EXECUTE_SCALAR(strSQL)
            strSQL = "SELECT "
            strSQL &= "SUM(CASE WHEN CSVファイル LIKE 'Leaflet%' THEN 1 ELSE 0 END) AS リーフ件数 "
            strSQL &= "FROM T_判定票_不備内容 "
            strSQL &= "WHERE ロットID = '" & strLotID & "' "
            strSQL &= "AND 不備種別 = 6 "
            strSQL &= "GROUP BY レコード番号"
            Dim iLeafletCount As Integer = sqlProcess.DB_EXECUTE_SCALAR(strSQL)
            strSQL = "SELECT "
            strSQL &= "SUM(CASE WHEN 帳票タイプ = 'BMI' THEN 1 ELSE 0 END) AS リーフBMI枚数 "
            strSQL &= "FROM T_判定票_不備内容 "
            strSQL &= "WHERE ロットID = '" & strLotID & "' "
            strSQL &= "AND 不備種別 = 6 "
            strSQL &= "GROUP BY レコード番号"
            Dim iLeafletBMICount As Integer = sqlProcess.DB_EXECUTE_SCALAR(strSQL)
            ...
            ExcelProcess.WriteExcelFile(ExcelOutputCategory.WorkOrder, strExcelFile, dtWeightCount, "",
                                        ...
                                        iCheckupCount, iLeafletCount, iLeafletCount + iLeafletBMICount)
    ...
    ''' <summary>
    ''' 印影不備となったレコードを[T_xxx_印影不備]テーブルに移動する
    ''' </summary>
    ''' <param name="strLotID"></param>
    ''' <param name="iRecordNum"></param>
    ''' <param name="strTableFrom"></param>
    ''' <param name="strTableTo"></param>
    Private Sub MoveRecordImprint(ByVal strLotID As String, ByVal iRecordNum As Integer, ByVal strTableFrom As String, ByVal strTableTo As String)
        ...
        Try
            '該当レコードを[T_判定票_印影不備]に移動する
            strSQL = "INSERT INTO " & strTableTo & " "
            strSQL &= "SELECT * FROM " & strTableFrom & " "
            strSQL &= "WHERE ロットID = '" & strLotID & "' "
            strSQL &= "AND レコード番号 = " & iRecordNum
            sqlProcess.DB_UPDATE(strSQL)
            '該当レコードを削除
            strSQL = "DELETE FROM " & strTableFrom & " "
            strSQL &= "WHERE ロットID = '" & strLotID & "' "
            strSQL &= "AND レコード番号 = " & iRecordNum
            sqlProcess.DB_UPDATE(strSQL)
            'T_判定票管理の該当レコードを「T_判定票管理_印影不備」に移動する
            strSQL = "INSERT INTO T_判定票管理_印影不備 "
            strSQL &= "SELECT * FROM T_判定票管理 "
            strSQL &= "WHERE ロットID = '" & strLotID & "' "
            strSQL &= "AND レコード番号 = " & iRecordNum
            sqlProcess.DB_UPDATE(strSQL)
            'T_判定票管理の該当レコードを削除
