...
#Region "オブジェクトイベント"
    ...
    Private Sub btnImport_Click(sender As Object, e As EventArgs) Handles btnImport.Click
        ...
        UpdateImportDate()
        Me.cmbLotID.SelectedValue = strLotID
        If Me.cmbLotID.SelectedIndex = -1 Then
...
#Region "プライベートメソッド"
    ...
    Private Sub SearchGridWeight(ByVal strLotID As String)
        ...
        Try
            ...
            strSQL = "SELECT COUNT(T1.レコード番号) FROM "
            strSQL &= "(SELECT レコード番号 FROM T_判定票_不備内容 "
            strSQL &= "WHERE ロットID = '" & strLotID & "' "
            strSQL &= "AND 不備種別 = 6 "
            strSQL &= "AND CSVファイル LIKE 'Checkup%' "
            strSQL &= "GROUP BY レコード番号) AS T1"
            ...
            strSQL &= "WHERE ロットID = '" & strLotID & "' "
            strSQL &= "AND 不備種別 = 6 "
            strSQL &= "AND CSVファイル LIKE 'Leaflet%' "
            strSQL &= "GROUP BY レコード番号) AS T1"
            Dim iLeafletCount As Integer = sqlProcess.DB_EXECUTE_SCALAR(strSQL)
            strSQL = "SELECT "
            strSQL &= "ISNULL(SUM(CASE WHEN CSVファイル LIKE 'Leaflet%' THEN 1 ELSE 0 END), 0) AS リーフ枚数 "
            strSQL &= "FROM T_判定票_不備内容 "
            strSQL &= "WHERE ロットID = '" & strLotID & "' "
            strSQL &= "AND 不備種別 = 6 "
            ...
            'strSQL &= "WHERE ロットID = '" & strLotID & "' "
            'strSQL &= "AND 不備種別 = 6"
            'Dim iLeafletBMISheet As Integer = sqlProcess.DB_EXECUTE_SCALAR(strSQL)

            If Me.C1FGridHeader.Rows.Count > 1 Then
                Me.C1FGridHeader(1, "判定票印影不備") = iCheckupCount
                Me.C1FGridHeader(1, "リーフ印影不備件") = iLeafletCount
                Me.C1FGridHeader(1, "リーフ印影不備枚") = iLeafletSheet
            End If
