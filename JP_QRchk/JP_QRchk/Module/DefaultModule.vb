...
#Region "フォルダ以下のファイルを検索または取得する"

    ' ''' <summary>
    ' ''' 指定した検索パターンに一致するファイルを検索し全て返す
    ' ''' </summary>
    ' ''' <param name="strRootPath">検索を開始する最上層のディレクトリへのパス</param>
    ' ''' <param name="strPatterns">パス内のファイル名と対応させる検索文字列</param>
    ' ''' <returns>検索パターンに一致したすべてのファイルパス</returns>
    ' ''' <remarks></remarks>
    ''<System.Diagnostics.DebuggerStepThrough()> _
    Public Function GetFiles(ByVal strRootPath As String, ByVal strPatterns() As String) As String()
