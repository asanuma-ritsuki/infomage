...
#Region "オブジェクトイベント"
	...
	Private Sub btnImport_Click(sender As Object, e As EventArgs) Handles btnImport.Click
		...
		Try
			...
			For Each strFile As String In strFilePathes
				...
				strSQL &= "WHERE 判定票CSV = '" & System.IO.Path.GetFileName(strFile) & "'"
			...
			For Each strFile As String In strTargetFiles
				...
				If ExtractZIP(strFile, strDestFolder, Me.txtPasswordDate.Text) Then
					...
					If sqlProcess.DB_EXECUTE_SCALAR(strSQL) = 0 Then
						'存在しなかった場合のみINSERT
						strSQL = "INSERT INTO T_ロット管理(ロットID, 発送日, 判定票CSV, リーフレットCSV, "
                    ...
                    Using parser As New CSVParser(strCheckupCSV(0), System.Text.Encoding.GetEncoding("Shift-JIS"))
                        ...
                        While Not parser.EndOfData
                            ...
                            For iCol As Integer = 0 To row.Count - 1
                                ...
                                '特定項目のみチェック
                                Select Case iCol
									'所属事業所、氏名セイ、氏名メイ、氏名姓、氏名名、受診検査機関名称、会場局名称、健診実施医師名、意見を述べた医師、判定医師名
			...
			If iIncompleteCount > 0 Or iDuplicateCount > 0 Then
				...
				strSaveFile = "データ不備内容_" & Me.cmbLotID.SelectedValue.ToString.Substring(0, 8) & ".xlsx"
	...
	Private Sub btnDeleteTest_Click(sender As Object, e As EventArgs) Handles btnDeleteTest.Click
		...
		If MessageBox.Show("指定されたロットIDを削除対象とします" & vbNewLine & "よろしいですか？", "確認", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.No Then
			Exit Sub
		End If
	...
	Private Sub btnPrintPreparation_Click(sender As Object, e As EventArgs) Handles btnPrintPreparation.Click
        ...
        If MessageBox.Show("印刷前準備処理を開始します" & vbNewLine & "よろしいですか？", "確認", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.No Then
            Exit Sub
        End If

		EnableControls(Me, False)
		PrintPreparation(Me.cmbLotID.SelectedValue, Me.lstResult, Me.txtLogFolder.Text)
...
#Region "プライベートメソッド"
	...
	Private Sub WeightHeaderToExcel(ByVal strExcelFile As String, ByVal strLotID As String)
		...
		Try
			...
			ExcelProcess.WriteExcelFile(ExcelOutputCategory.WeightCount, strExcelFile, dtWeightCount, "")
