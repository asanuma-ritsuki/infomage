...
#End Region
            ...
            For iRow As Integer = 0 To dtOffice.Rows.Count - 1
                ...
                For iSystemID As Integer = 0 To dtLeadletSystemID.Rows.Count - 1
                    ...
                    For i As Integer = 0 To dtLeafletPrint.Rows.Count - 1
                        ...
                        strSQL &= ", '" & strWAS6 & "'"
                        'QRコードの算出(システムID(15)＋帳票種別ID(1)＋エラーフラグ(1))
                        strQRCodeWithoutCD = dtLeafletPrint.Rows(i)("システムID") & "5" & iErrorFlag.ToString & (i + 1).ToString   '←エラーフラグ付加※カレントSEQ付加
                        strCD = ""
                        blnResult = Modulus10(strQRCodeWithoutCD, 5, strCD)
                        strQRCode = strQRCodeWithoutCD & strCD
    ...
    ''' </summary>
    ''' <param name="strLotID"></param>
    Public Sub PrintLabel(ByVal strLotID As String, ByVal blnPrintWeightHeader As Boolean, ByVal strWeightHeader As String)
        ...
        Try
            ...
            strSQL &= "T4.郵便番号, T4.住所, T1.QR, T2.重量ヘッダ, T3.備考 AS 封筒サイズ, T2.ラベル連番 "
    ...
    Public Sub PrintCheckup(ByVal strLotID As String, ByVal blnPrintWeightHeader As Boolean, ByVal strWeightHeader As String, Optional ByVal iRecordNumber As Integer = 0)
        ...
        Try
            ...
            If iRecordNumber = 0 Then
                ...
                'strSQL &= "AND 会社コード = '3001' "
                'strSQL &= "AND 所属事業所コード = '780150' "
                ...
                For iRow As Integer = 0 To dtPrintManage.Rows.Count - 1
                    ...
                    If dt.Rows.Count > 0 Then
                        ...
                        '判定票印刷日時書き込み
                        WritePrintDate(strLotID, PrintCategory.Checkup, 0, dtPrintManage.Rows(iRow)("会社コード"), dtPrintManage.Rows(iRow)("所属事業所コード"))
                    End If
        ...
        Try
            ...
            If iRecordNumber = 0 Then
                ...
                For iRow As Integer = 0 To dtPrintManage.Rows.Count - 1
                    ...
                    strSQL &= "GROUP BY T1.ロットID, T1.レコード番号, T1.所属部課名, T3.社員コード "
                    ...
                    For i As Integer = 0 To dtLeaflet.Rows.Count - 1
                        ...
                        strSQL &= "T1.局所会社名, T1.健康管理施設名, T1.郵便番号, T1.住所, T1.電話番号, T1.年度, T1.帳票タイプ, "
                        strSQL &= "T1.結果値1, T1.結果値2, T1.結果値3, T1.BOC, T1.EOC, T1.PAR, T1.WAS1, T1.WAS2, T1.WAS3, T1.WAS4, T1.WAS5, T1.WAS6, "
                        strSQL &= "T1.QRコード, T4.ラベル連番, T5.ID "
                        strSQL &= "ORDER BY T5.ID"
                        ...
                        For j As Integer = 0 To dt.Rows.Count - 1
                            '帳票タイプごとに回す
                            '印刷処理
                            Print(strSQL, PrintCategory.Leaflet, dt.Rows(j)("帳票タイプ"))
                    ...
                    'リーフレット印刷日時書き込み
                    WritePrintDate(strLotID, PrintCategory.Leaflet, 0, dtPrintManage.Rows(iRow)("会社コード"), dtPrintManage.Rows(iRow)("所属事業所コード"))
        ...
        Try
            ...
            Select Case prtCategory
				...
				Case PrintCategory.Label
                    ...
                    '給紙トレイを変更する
                    'print_option.PrinterSettings.DefaultPageSettings.PaperSource = print_option.PrinterSettings.PaperSources.Item(XmlSettings.Instance.ResultTray)    'トレイ1※対象者一覧のトレイと同一
				...
				Case PrintCategory.WeightHeader
					...
					'給紙トレイを変更する
					print_option.PrinterSettings.DefaultPageSettings.Landscape = False  '縦向き
                    'print_option.PrinterSettings.DefaultPageSettings.PaperSource = print_option.PrinterSettings.PaperSources.Item(XmlSettings.Instance.SentlistTray)    'トレイ1※対象者一覧のトレイと同一
                    '==================================================
	...
	Private Sub WritePrintDate(ByVal strLotID As String, ByVal prtCategory As PrintCategory, Optional ByVal iRecordNumber As Integer = 0, Optional ByVal strCompanyCode As String = "", Optional ByVal strOfficeCode As String = "")
