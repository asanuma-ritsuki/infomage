...
namespace ConvertWideNarrow
{
    internal static class DefaultModule
    {
        ...
        /// <summary>
        /// リストボックスに表示されている文字列をログファイルとして保存する（出力ファイル指定）
        /// </summary>
        /// <param name="lstResult"></param>
        /// <param name="strLogFile"></param>
        /// <param name="enc"></param>
        public static void OutputListLog(ListBox lstResult, string strLogFile, Encoding enc)
        {
            using (StreamWriter sw = new StreamWriter(strLogFile, false, enc))
            {
                // テキストの書き込み
            ...
            WriteLstResult(lstResult, "ログ保存：" + strLogFile);
        }
